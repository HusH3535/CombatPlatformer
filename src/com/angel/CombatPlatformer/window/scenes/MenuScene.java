...
public class MenuScene extends Scene{
    ...
    public MenuScene(){
        try{
            BufferedImage spritesheet = ImageIO.read(new File("src/com/angel/CombatPlatformer/assets/blafiAdMenuSprite.png"));
            play = spritesheet.getSubimage(
                    playButtonSpritePos.x,
                    playButtonSpritePos.y,
                    playButtonSpritePos.w,
                    playButtonSpritePos.h);
            playHover = spritesheet.getSubimage(
                    playButtonHoverSpritePos.x,
                    playButtonHoverSpritePos.y,
                    playButtonHoverSpritePos.w,
                    playButtonHoverSpritePos.h);
            playCurrent = play;

            quit = spritesheet.getSubimage(
                    quitButtonSpritePos.x,
                    quitButtonSpritePos.y,
                    quitButtonSpritePos.w,
                    quitButtonSpritePos.h);
            quitHover = spritesheet.getSubimage(
                    quitButtonHoverSpritePos.x,
                    quitButtonHoverSpritePos.y,
                    quitButtonHoverSpritePos.w,
                    quitButtonHoverSpritePos.h);
            quitCurrent = quit;

            background = spritesheet.getSubimage(
                    backgroundSpritePos.x,
                    backgroundSpritePos.y,
                    backgroundSpritePos.w,
                    backgroundSpritePos.h);
        ...
        playButtonHitBox = new Rect(
                (int) ButtonScreenXPos,
                (int) windowHeight20per,
                (int) windowWidth20per,
                (int) windowHeight20per);

        quitButtonHitBox = new Rect(
                (int) ButtonScreenXPos,
                (int) (windowHeight20per) * 2 + 10,
                (int) windowWidth20per,
                (int) windowHeight20per);
